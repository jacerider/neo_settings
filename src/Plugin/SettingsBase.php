...
abstract class SettingsBase extends PluginBase implements SettingsInterface, TrustedCallbackInterface, ContainerFactoryPluginInterface {
   ...
   * The strict parents.
   *
   * An array of settings-relative key paths whose values are used as-is instead
   * of deep merging. Declare any key holding a list — `checkboxes`, a multiple
   * `select`, or a table — because mergeDeepStrict() tests `is_int(key($v))`
   * and `key([])` is NULL, so an empty array would recurse and be swallowed,
   * silently restoring the previous value when a user clears the field.
   *
   * @var array
   */
